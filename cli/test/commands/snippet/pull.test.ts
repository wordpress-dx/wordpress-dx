...
describe('pull helpers', () => {
  describe('buildSnippetFile', () => {
    it('prepends <?php when PHP code has no opening tag', () => {
      ...
      expect(buildSnippetFile(snippet)).to.equal("<?php\n\nadd_filter('x', 'y');")
    })
  ...
  describe('buildMetaFile', () => {
    it('includes required fields', () => {
      ...
      expect(meta.name).to.equal('My Snippet')
      expect(meta.type).to.equal('php')
      expect(meta.active).to.be.false
    })

    it('omits description when empty', () => {
      const meta = JSON.parse(buildMetaFile({...base, description: ''}))
      expect(meta).not.to.have.property('description')
    })
    ...
    it('omits tags when empty', () => {
      ...
      expect(meta).not.to.have.property('tags')
    })

    it('includes tags when present', () => {
      const meta = JSON.parse(buildMetaFile({...base, tags: ['sample', 'dates']}))
      expect(meta.tags).to.deep.equal(['sample', 'dates'])
    })
